...
class OpenAIService:
    def __init__(self):
        self.model = "gpt-4-1106-preview"  # Latest model for better JSON structuring
    ...
    async def process_chunk(self,
        ...
        try:
            ...
            try:
                ...
                if retry_count < 2:  # Retry with more explicit instructions
                    logger.warning(f"JSON parse error, retrying chunk: {str(e)}")
                    return await self.process_chunk(chunk, previous_context, retry_count + 1)
                else:
                    raise HTTPException(status_code=500, detail="Failed to parse response into JSON")
    ...
    def validate_and_clean_json(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(cleaned["applicable_drug_list"], str):
            cleaned["applicable_drug_list"] = [x.strip() for x in cleaned["applicable_drug_list"].split(",") if
                                               x.strip()]
